...
<script>
  ...
  export default {
    ...
    mixins: [postCat],
    mounted () {
      this.$http.get('https://api.thecatapi.com/api/images/get?format=xml&results_per_page=1').then(response => {
        console.log(parse(response.body))
        const catUrl = parse(response.body).root.children['0'].children['0'].children['0'].children['1'].content
        let img = new Image()
